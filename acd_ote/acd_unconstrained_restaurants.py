...
def classify(traincorpus, testcorpus):
    ...
    if just_save_features:
        save_features(train_features1, 'acd-rest-train')
    else:
    ...
    if just_save_features:
        save_features(test_features1, 'acd-rest-test')
        exit(0)
    ...
    for i, test_fvector1 in enumerate(test_features1):
        ...
        entity_prob = {"food": (food_pred1 + food_pred2) / 2, "drinks": (drinks_pred1 + drinks_pred2) / 2,
                       "service": (service_pred1 + service_pred2) / 2,
                       "ambience": (ambience_pred1 + ambience_pred2) / 2,

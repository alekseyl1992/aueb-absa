...
class features():
    def __init__(self, train_path, test_path, dom):
        ...
        if dom == 'rest':
            ...
            self.entities = ['LAPTOP', 'DISPLAY', 'KEYBOARD', 'MOUSE', 'MOTHERBOARD', 'CPU', 'FANS_COOLING', 'PORTS',
                             'MEMORY', 'POWER_SUPPLY', 'OPTICAL_DRIVES', 'BATTERY',
                             'GRAPHICS', 'HARD_DISK', 'MULTIMEDIA_DEVICES', 'HARDWARE', 'SOFTWARE', 'OS', 'WARRANTY',
                             'SHIPPING', 'SUPPORT', 'COMPANY']
            self.attributes = ['GENERAL', 'PRICE', 'QUALITY', 'OPERATION_PERFORMANCE', 'USABILITY', 'DESIGN_FEATURES',
    ...
    def listCaps(self, text):
        ctr = [',', '<', '.', '>', '/', ';', ':', ']', '}', '[', '{', '|', '@', '$', '%', '^', '&', '*', '(', ')', '_',
               '-', '+', '+', '"', '1', '2', '3', '4', '5', '6', '7', '8', '9']  # removing characters
        l = []
        counter = 0
        for word in text:
        ...
        for bigram in bigrams:
            ...
            for i in range(len(bigram)):
                for j in range(len(self.train_posBigram_lexicon)):
                    if (bigram[i][0].lower() == self.train_posBigram_lexicon[j][0][0]) and (
                        bigram[i][1].lower() == self.train_posBigram_lexicon[j][0][1]):
                        ...
                        if (pre_max_pos < self.train_posBigram_lexicon[j][1][
                            0]):  # calculating the maximum precision for the positive
                            pre_max_pos = self.train_posBigram_lexicon[j][1][0]
                        if (pre_min_pos > self.train_posBigram_lexicon[j][1][
                            0]):  # calculating the minimum precision for the positive
                            pre_min_pos = self.train_posBigram_lexicon[j][1][0]

                        if (pre_max_neg < self.train_posBigram_lexicon[j][1][
                            1]):  # calculating the maximum precision for the negative
                            pre_max_neg = self.train_posBigram_lexicon[j][1][1]
                        if (pre_min_neg > self.train_posBigram_lexicon[j][1][
                            1]):  # calculating the minimum precision for the negative
                            pre_min_neg = self.train_posBigram_lexicon[j][1][1]

                        if (pre_max_neu < self.train_posBigram_lexicon[j][1][
                            2]):  # calculating the maximum precision for the neutral
                            pre_max_neu = self.train_posBigram_lexicon[j][1][2]
                        if (pre_min_neu > self.train_posBigram_lexicon[j][1][
                            2]):  # calculating the minimum precision for the neutral
                            pre_min_neu = self.train_posBigram_lexicon[j][1][2]

                        if (f1_max_pos < self.train_posBigram_lexicon[j][1][
                            3]):  # calculating the maximum f1 for the positive
                            f1_max_pos = self.train_posBigram_lexicon[j][1][3]
                        if (f1_min_pos > self.train_posBigram_lexicon[j][1][
                            3]):  # calculating the minimum f1 for the positive
                            f1_min_pos = self.train_posBigram_lexicon[j][1][3]

                        if (f1_max_neg < self.train_posBigram_lexicon[j][1][
                            4]):  # calculating the maximum f1 for the negative
                            f1_max_neg = self.train_posBigram_lexicon[j][1][4]
                        if (f1_min_neg > self.train_posBigram_lexicon[j][1][
                            4]):  # calculating the minimum f1 for the negative
                            f1_min_neg = self.train_posBigram_lexicon[j][1][4]

                        if (f1_max_neu < self.train_posBigram_lexicon[j][1][
                            5]):  # calculating the maximum f1 for the neutral
                            f1_max_neu = self.train_posBigram_lexicon[j][1][5]
                        if (f1_min_neu > self.train_posBigram_lexicon[j][1][
                            5]):  # calculating the minimum f1 for the neutral
                ...
                if (len(sentence) > 1):
                    ...
                    if (len(opinions) > 0):  # check if there are aspects
                        ...
                        for opinion in opinions:
                            ...
                            temp0 = self.checkLexiconReady(text, self.stopwords, capsList, self.negation,
                                                           self.AFINN_lexicon)  # afinn lexicon scores
                            temp3 = self.checkLexiconReady(text, self.stopwords, capsList, self.negation,
                                                           self.huLiu_lexicon)  # Hu and Liu lexicon scores
                            temp4 = self.checkLexiconReady(text, self.stopwords, capsList, self.negation,
                                                           self.nrc_lexicon)  # NRC lexicon scores

                            temp1 = self.checkLexiconUni(text, chars_to_remove, self.stopwords, capsList, self.negation,
                                                         self.train_unigram_lexicon)  # unigram lexicon scores from the train data of each domain

                            temp7 = self.howManyUpperFirst(t2)  # num of words starting with capitalized first letter
                            temp9 = [self.howMany(text, '?'),
                                     self.howMany(text, '!')]  # number of question and exclamation marks

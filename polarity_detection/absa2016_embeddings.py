        ...
        for sentence in sentences:  # for each sentence of the coprus
            ...
            for word0 in sentence:  # for each word of the sentence
                ...
                for i in range(len(word0)):
                    word = word0[i]
                    word = ''.join(
                        [c for c in word if c not in chars_to_remove])  # handles cases like '1.Food' --> 'Food'
            ...
            if l == 0:  # if no word of the sentence has an embedding
                ...
                embeddings = self.normalize(
                    embeddings)  # normalize the embedding vector before calculating the centroid of the sentence
                ...
                centroid /= l  # caclulating the average
